...
% Формат номеров разделов без завершающей точки
...
\captionsetup[table]{%
  name={Таблица},          % слово перед номером
  labelsep=space,          % разделитель: пробел (без точки и двоеточия)
  justification=raggedright, % выравнивание по левому полю
  singlelinecheck=false,   % не центрировать однострочные подписи
  font=normalsize,         % обычный шрифт (без \bfseries)
  labelfont=normalsize     % номер/имя таблицы тоже не жирным
}
...
% страница содержания = страница 1
\setcounter{page}{1}
\pagestyle{plain}
